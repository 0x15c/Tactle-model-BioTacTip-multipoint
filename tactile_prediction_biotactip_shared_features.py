...
from tactmorph_shared_features.model import SharedFeatureTactMorph2D
...
def get_shared_feature_model(weights_path: str, device: torch.device, cfg: Config) -> SharedFeatureTactMorph2D:
    checkpoint = Path(weights_path)
    if not checkpoint.exists():
        raise FileNotFoundError(f"Shared-feature TactMorph checkpoint not found: {checkpoint}")
    ...
    model = SharedFeatureTactMorph2D(base_channels=base_channels)
    model.load_state_dict(state["model_state_dict"], strict=True)

    model.to(device)
    model.eval()
    total_params, trainable_params = count_parameters(model)
    param_size_mb = total_params * 4.0 / (1024.0 * 1024.0)
    print(
        "Shared-feature TactMorph parameters: "
...
def main(cfg: Config):
    ...
    print(f"Shared-feature model preprocessing: {cfg.model_preprocess}, input size: {cfg.model_input_size}")
    model = get_shared_feature_model(cfg.model_weights_path, device, cfg)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Online shared-feature TactMorph displacement visualization for tactile sensor images")
    ...
    parser.add_argument("--weights", type=str, default=None, help="shared-feature TactMorph checkpoint path")
