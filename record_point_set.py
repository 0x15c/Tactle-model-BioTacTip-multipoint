...
class grad_descent():
    def __init__(self, arrZ, seeds, cropped_size):
        ...
        try:
            for i in range(iters):
                ...
                self.iter_pts = self.iter_pts + \
                    grad * np.exp(-alpha * i + beta)
    ...
    def grad(self, pts, step):
        try:
            pts_limit_mask = (pts[:, 0] <= self._cropped_size[0] - step) & (
                pts[:, 1] <= self._cropped_size[1] - step) & (pts[:, 0] >= 0) & (pts[:, 1] >= 0)
            pts = pts[pts_limit_mask]
            self.iter_pts = self.iter_pts[pts_limit_mask]
            gradY = 1 / (2 * step) * (self.arrZ[pts[:, 1] + step,
                                                pts[:, 0]] - self.arrZ[pts[:, 1] - step, pts[:, 0]])
            gradX = 1 / \
                (2 * step) * (self.arrZ[pts[:, 1], pts[:, 0] +
                                        step] - self.arrZ[pts[:, 1], pts[:, 0] - step])
...
# ---------------- Main processing ----------------
training_file_path = "./training_biotactip.h5"
...
def main(cfg: Config):
    ...
    video_w, video_h = int(cap.get(cv.CAP_PROP_FRAME_WIDTH)), int(
    ...
    o_ring_mask = create_radial_mask(
        (cfg.image_size, cfg.image_size), max_value=245)

    out_original = cv.VideoWriter(
        cfg.output_original_path, fourcc, capture_fps, tuple(cropped_size), False)
    heatmap_output = cv.VideoWriter(
        cfg.output_heatmap_path, fourcc, capture_fps, tuple(cropped_size), True)
    ...
    while True:
        ...
        frame_cropped = frame[cropped_limits[0][1]:cropped_limits[1]
                              [1], cropped_limits[0][0]:cropped_limits[1][0]]
        cropped_frame = cv.bitwise_and(
            frame_cropped, frame_cropped, mask=c_mask)
        ...
        _, frame_binary = cv.threshold(
            grey_frame_corrected, cfg.threshold_bin, 255, cv.THRESH_BINARY)
        ...
        if pts is None:
            ...
            cluster_data = DBSCAN(
                eps=cfg.dbscan_eps_points, min_samples=cfg.dbscan_min_samples_points).fit(cluster_coordinates)
        ...
        if frame_count >= cfg.frame_skip_init and centroids.size and centroids_init.size:
            c0n, cn = truncate_point_sets(centroids_init, centroids)
            with h5py.File(training_file_path, 'a') as f:
                # Create a group for the current frame
                group_name = f'frame_{frame_count}'
                group = f.create_group(group_name)

                # Save both pointsets inside that group
                group.create_dataset('c0', data=c0n, compression="gzip")
                group.create_dataset('c', data=cn, compression="gzip")
            pass
        frame_count += 1
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Online heatmap visualization for tactile sensor markers')
    parser.add_argument('--input', type=int, default=0,
                        help='capture device index or file path')
    parser.add_argument('--orig-out', type=str, default='videos/output_original.mp4',
                        help='path to save original grayscale capture')
    parser.add_argument('--heat-out', type=str,
                        default='videos/output_from_online_cap.mp4', help='path to save heatmap video')
